...
namespace z3y.Shaders
{
    public class BaseShaderGUI : ShaderGUI
    {
        ...
        public sealed override void OnGUI(MaterialEditor materialEditor, MaterialProperty[] materialProperties)
        {
        ...
        public bool Foldout(ref bool foldout, string text)
        {
            DrawSplitter();
            foldout = DrawHeaderFoldout( new GUIContent (text), foldout);
            if (foldout)
            {
                EditorGUILayout.Space();
            }
        ...
        public void ResetProperty(Shader shader, MaterialProperty[] materialProperty)
        {
            for (int i = 0; i < materialProperty.Length; i++)
            {
                ResetProperty(shader, materialProperty[i]);
            }
        }

        public void ResetProperty(Shader shader, MaterialProperty materialProperty)
        {
            int propIndex = shader.FindPropertyIndex(materialProperty.name);

            switch (materialProperty?.type)
            {
                case MaterialProperty.PropType.Vector:
                    materialProperty.vectorValue = shader.GetPropertyDefaultVectorValue(propIndex);
                    break;
                case MaterialProperty.PropType.Color:
                    materialProperty.colorValue = shader.GetPropertyDefaultVectorValue(propIndex);
                    break;
                ...
                case MaterialProperty.PropType.Range:
                    materialProperty.floatValue = shader.GetPropertyDefaultFloatValue(propIndex);
                    break;
            }
        }
    ...
    public static class BaseShaderGUIExtensions
    {
